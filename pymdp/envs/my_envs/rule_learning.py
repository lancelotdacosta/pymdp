...
2. Colour - the correct color state
3. Where - where attention or saccadic eye movements are directed
4. Choice - the (manual) response

Note: this environment is slightly different from the version of the rule learning environment in the rule learning paper, for two reasons:
 - the likelihood mapping for colors is deterministic, not uniform (like in the paper, c.f Figure 7). This means that the color of the locations that are not the rule location are fixed in each trial, so that if the agent looks at that location twice or more it will always receive the same visual outcome.
 - there are eight actions here: the agent can choose one of the four locations and choices at each timepoint, while in the original paper there are six actions: the agent can go to each of the three non-centre locations and be undecided or it can go to the centre and make a choice
"""
...
class RuleLearningEnv(Env):
    ...
    def reset(self, state=None):
        ...
        if state is None:
            ...
            rule_idx = rule_state.argmax()
            if rule_idx == 1:  # If top rule
                colour_state = utils.onehot(1, self.num_colours)  # Set color to green
            else:
    ...
    def _construct_likelihood_dist(self):
        ...
        # Randomly assign color to non-rule locations (indices 0 or 2)
        # Colors are 0,1,2 (excluding 3 which is white)
        non_rule_colors = np.random.choice(3, size=2, replace=True)  # Random colors for the non-rule location (there may be one or two of such locations depending on the rule)
            ...
            for colour in range(self.num_colours):
                for where in range(self.num_locations):
                    for choice in range(self.num_choices):
                        ...
                        if where == CENTRE:  # Center location
                            A[WHAT_OBS_ID][3, rule, colour, where, choice] = 1.0  # Always white
                        elif where == TOP:  # Colour Top location 
                            A[WHAT_OBS_ID][rule, rule, colour, where, choice] = 1.0 # represents rule (rule left -> red on top, rule top -> green on top, rule right -> blue on top)
                        else: #location left or right
                            if where == rule: #if rule location
                                A[WHAT_OBS_ID][colour, rule, colour, where, choice] = 1.0 #show true color
                            else: #non rule location
                                A[WHAT_OBS_ID][non_rule_colors[int(where/2)], rule, colour, where, choice] = 1.0 #show random color

                        # Feedback observation depends on rule and choice
                        if choice != 3:  # If a choice has been made
                            correct_choice = self._evaluate_choice(rule, colour, where, choice) #looks at whether choice color and true color coincide
                            if correct_choice:
    ...
    def _evaluate_choice(self, rule, colour, where, choice):
        ...
        Choices: 0: Red, 1: Green, 2: Blue, 3: Undecided
    ...
    def construct_policies(self, policy_len=1):
        ...
        Returns
        -------
        valid_policies: list of numpy.ndarray
            List of valid policies where non-undecided choices are only made at the center location.
        """
        all_policies = control.construct_policies(num_states=self.num_states,
                                                        num_controls = self.num_controls, 
                                                        policy_len=policy_len,
                                                        control_fac_idx=[WHERE_FACTOR_ID, CHOICE_FACTOR_ID])
        valid_policies = []
    ...
    def print_q_s(self, q_s, inference_algo, timestep, policy_len=1):
        """
        Print routine for beliefs about states based on inference algorithm
        ...
        policy_len : int, optional
            Length of policies (default=1)
        """
        def print_q_s_t(q_s, timestep):
        ...
        if inference_algo == "VANILLA":
            print_q_s_t(q_s, timestep)  
        elif inference_algo == "MMP":  # MMP
            ...
            for tau in range(effective_inference_horizon):  # Loop through all past timesteps up to present (inclusive)
                ...
                if abs_time >= 0:
                    print_q_s_t(q_s[0][tau], abs_time) # Get beliefs from first policy (past and present beliefs are invariant under policies)
